...
class CorrectionEvaluator:
    ...
    def load_correction_rules(self, language: Language) -> list[dict[str, str]]:
        """Load correction rules for a language with caching. Uses core.rules_loader."""
        if language in self.rules_cache:
            return self.rules_cache[language]
        lang_code = language.value
        raw = load_rules_from_core(lang_code, self.rules_dir)
        rules = [
            {
                "biased": r.get("biased", ""),
                "neutral_primary": r.get("neutral_primary", ""),
                "severity": r.get("severity", "replace"),
            }
            for r in raw
        ]
