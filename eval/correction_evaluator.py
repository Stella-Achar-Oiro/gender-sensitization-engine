...
class SemanticPreservationMetrics:
    ...
    def calculate_bleu_score(original: str, corrected: str, n: int = 2) -> float:
        ...
        for gram_size in range(1, n + 1):
            orig_ngrams = [tuple(orig_tokens[i:i+gram_size]) 
                          for i in range(len(orig_tokens) - gram_size + 1)]
            corr_ngrams = [tuple(corr_tokens[i:i+gram_size]) 
...
class CorrectionEvaluator:
    ...
    def evaluate_correction_effectiveness(
            ...
            for gt_sample in ground_truth:
                ...
                if text != corrected_text:
                    preservation_metrics = self.semantic_metrics.calculate_composite_preservation_score(
                        text, corrected_text
                    )
                ...
                if is_biased and bias_removed:
                    ...
                    if preservation_metrics and preservation_metrics['composite_score'] >= self.GOOD_PRESERVATION_THRESHOLD:
            ...
            if pre_f1 > 0 and removal_rate > 0:
                results['overall_metrics']['harmonic_score'] = harmonic_mean([pre_f1, removal_rate])
            else:
                results['overall_metrics']['harmonic_score'] = 0.0

            # Calculate average semantic preservation scores
            if preservation_scores:
                results['semantic_preservation']['samples_analyzed'] = len(preservation_scores)
            ...
            for category, cat_data in category_data.items():
                ...
                if cat_data['preservation_scores']:
                    ...
                    cat_preservation = {
                        'avg_composite': sum(s['composite_score'] for s in pres_scores) / len(pres_scores),
                        'avg_bleu': sum(s['bleu_score'] for s in pres_scores) / len(pres_scores),
    ...
    def generate_comparison_report(self, results: dict[str, Any]) -> str:
        ...
        if harmonic_score >= self.GOOD_HARMONIC_SCORE_THRESHOLD:
            ...
            report += f"  → Assessment: GOOD\n"
        elif harmonic_score >= 0.40:
            report += f"  → Assessment: FAIR\n"
        else:
            report += f"  → Assessment: NEEDS IMPROVEMENT\n"
        report += "\n"
        ...
        if results['semantic_preservation']['samples_analyzed'] > 0:
            ...
            if pres['avg_composite_score'] >= self.GOOD_PRESERVATION_THRESHOLD:
                report += f"  → Assessment: EXCELLENT preservation\n"
            elif pres['avg_composite_score'] >= 0.70:
                report += f"  → Assessment: GOOD preservation\n"
            else:
                report += f"  → Assessment: Moderate preservation, review needed\n"
            report += "\n"
        ...
        report += f"  High-Quality Corrections:   {quality['high_quality_corrections']}\n"
        report += f"  Over-Corrections:           {quality['over_corrections']}\n"
        report += f"  Meaning Preserved (manual): {quality['meaning_preserved']} samples\n\n"
...
if __name__ == "__main__":
    ...
    for lang in Language:
        ...
        output_file = Path(f"eval/results/correction_evaluation_{lang.value}_{timestamp}.json")
        evaluator.save_results_to_json(results, output_file)

        report_file = Path(f"eval/results/correction_report_{lang.value}_{timestamp}.txt")
