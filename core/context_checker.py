...
class ContextChecker:
    ...
    CONTEXT_PATTERNS: Dict[ContextCondition, List[str]] = {
        ...
        ContextCondition.HA_NEUTRAL_BUDURWA: [
            # Bare budurwa inside fixed phrase wata budurwa — handled by separate lexicon row (warn).
            r'\bwata\s+{term}\b',
            # Questions / discourse (shin only — avoids blocking "Ko … balle budurwa!!" sermons).
            ...
            # Compound / colloquial neutrals (StudyLabs social & health scroll)
            r'\byarinya\s+{term}\b',
            r'\bmaganan\s+{term}\b',
            r'\bwai\s+{term}\b',
            r'\bkamar\s+{term}\b',
