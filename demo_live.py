...
def demo_example(detector, text, language, example_num):
    ...
    print(f"\nDetection Result:")
    print(f"   Bias Detected: {'YES [WARNING]' if result.has_bias_detected else 'NO [OK]'}")
    print(f"   Detection Method: Rules-based (Hybrid System)")

    if result.detected_edits:
        print(f"\nSuggested Corrections:")
        for i, edit in enumerate(result.detected_edits, 1):
            print(f"   {i}. \"{edit['from']}\" -> \"{edit['to']}\" (severity: {edit.get('severity', 'replace')})")
